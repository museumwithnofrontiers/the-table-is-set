...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('theTableIsSet.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('theTableIsSet.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('theTableIsSet.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
